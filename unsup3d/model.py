...
class Classification():
    def __init__(self, cfgs):
        ...
        self.net = networks.Encoder(cin=3, cout=2, nf=32, activation=None)
    ...
    def forward(self, input, label, **kwargs):
        ...
        output = self.net(self.input_im)

        self.loss_total = nn.functional.cross_entropy(output, label)

        pred = torch.argmax(output, -1)
        acc = (pred == label).float().mean()

        metrics = {'loss': self.loss_total, "acc": acc}
...
class Unsup3D_Classifier():
    def __init__(self, cfgs):
        ...
        ## networks and optimizers
        self.netD = networks.EDDeconv(cin=3, cout=1, nf=64, zdim=256, activation=None)
        self.netA = networks.EDDeconv_emb(cin=3, cout=3, nf=64, zdim=256)
    ...
    def forward(self, input, label, *args, **kwargs):
        ...
        ## predict canonical albedo
        self.canon_albedo, emb = self.netA(self.input_im)  # Bx3xHxW
        self.canon_albedo = torch.cat([self.canon_albedo, self.canon_albedo.flip(3)], 0)  # flip
        ...
        output = self.netClassifier(emb)

        self.loss_total = self.loss_total + nn.functional.cross_entropy(output, label)
